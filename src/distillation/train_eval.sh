...
BASE_TRAIN_REPORT=modelv1-KaggleDRD-50ep-base-distillation-train.json
BASE_TRAIN_ROC=modelv1-KaggleDRD-50ep-base-distillation-train.png
...
#train using distillation on incremental data
#python train_incremental_distillation.py --train_dir $TRAIN_DIR --validation_dir $VAL_DIR --prev_model $PREV_MODEL --num_epochs $EPOCHS --batch_size $BATCH_SIZE --image_height $IMAGE_HEIGHT --image_width $IMAGE_WIDTH --checkpoint_file $CKPT --log_file $LOG --train_logits_file $TRAIN_LOGITS_FILE --val_logits_file $VAL_LOGITS_FILE
