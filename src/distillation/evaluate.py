...
from train_incremental_distillation import knowledge_distillation_loss, accuracy, temperature, lambda_const

logits_dir = '../../data/base_data/'

def evaluation_report(data_dir, logits_file, checkpoint, report_file, batch_size, input_shape, extract_features=False, feature_file=None):
  ...
  if 'distillation' in checkpoint:
    model = load_model(checkpoint, custom_objects={'<lambda>': lambda y_true, y_pred: knowledge_distillation_loss(y_true, y_pred, lambda_const), 'accuracy': accuracy, 'temperature': temperature})
  else:
