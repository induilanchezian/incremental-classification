...
def distillation_loss(model_obj): 
  preds_layer_model = Model(inputs=model_obj.prev_model.input,
                                     outputs=model_obj.prev_model.layers[-1].output)
  preds = preds_layer_model.predict(model_obj.model.input)
  def loss(y_true, y_pred):
    return K.binary_crossentropy(y_true, y_pred) + K.binary_cross_entropy(preds, y_pred)
  return loss
...
class IncrementalModel:
  ...
  def previous_model(self):
    if 'distillation' in self.latest_model:
      self.prev_model = load_model(self.latest_model, custom_objects={'loss': distillation_loss(self)})
    else:
  ...
  def train(self):
    ...
    if 'distillation' in self.latest_model:
      self.model = load_model(self.latest_model, custom_objects={'loss': distillation_loss(self)})
    else:
      self.model = load_model(self.latest_model)
    self.model.compile(loss=distillation_loss(self),
              optimizer=sgd,
              metrics=['accuracy'])
