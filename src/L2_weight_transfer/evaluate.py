...
from l2_transfer_incremental_train import l2_loss
import os
import glob
import argparse
import json
...
def evaluation_report(data_dir, checkpoint, report_file, batch_size, input_shape, extract_features=False, feature_file=None, roc_file=None):
  ...
  default_model = load_model('../../models/base/modelv1-KaggleDRD-50ep-base.h5')
  lambda_const = 0.1
  model = load_model(checkpoint, custom_objects={'<lambda>':lambda y_true, y_pred: l2_loss(y_true, y_pred, default_model.layers, default_model.layers, lambda_const)})
