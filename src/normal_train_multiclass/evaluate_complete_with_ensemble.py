...
def predict_binary(test_data_dir, checkpoint, input_shape, batch_size):
  nb_test_samples = np.sum([len(glob.glob(test_data_dir+'/'+i+'/*.jpeg')) for i in ['0','1','2','3','4']])
...
def predict(test_data_dir, checkpoint, input_shape, batch_size):
  nb_test_samples = np.sum([len(glob.glob(test_data_dir+'/'+i+'/*.jpeg')) for i in ['0','1','2','3','4']])
  test_steps = nb_test_samples // batch_size
...
def evaluation_report(test_data_dir, checkpoints_list, report_file, batch_size, input_shape, extract_features=False, feature_file=None, roc_file=None):
  pred, true_labels, classes = predict_binary(test_data_dir, checkpoints_list[0], input_shape, batch_size)
  pred2, true_labels, classes = predict_binary(test_data_dir, checkpoints_list[1], input_shape, batch_size)
  pred3, true_labels, classes = predict_binary(test_data_dir, checkpoints_list[2], input_shape, batch_size)
  predictions = np.zeros((len(pred), len(checkpoints_list)-1))
  predictions_stage_0 = (pred + pred2 + pred3).astype(float) / 3.0 
  predictions_stage_0 = predictions_stage_0.astype(int)
  for j, c in enumerate(checkpoints_list[3:]):
    pred  = predict(test_data_dir, c, input_shape, batch_size)
    predictions[:,j] = pred
  ...
  print(predictions_stage_0)
  y_pred = y_pred.astype(int)
  y_pred = y_pred + 1
  predictions_stage_0[predictions_stage_0 == 1] = y_pred[predictions_stage_0 == 1]
