...
TRAIN_DIR=../../data/preprocessed_2c_balanced/train
...
IMAGE_HEIGHT=512
IMAGE_WIDTH=512
CKPT_PATH=weights_binary_normal_vs_mild_smaller_3 
CKPT=../../models/base/
CKPT_FILE=$CKPT/weights_binary_normal_vs_mild_smaller_3.50-1.18.hdf5
LOG=losses_mc_1.txt
...
CUDA_VISIBLE_DEVICES=$DEVICE python base_train_multiclass_model2_binary.py --train_dir $TRAIN_DIR --validation_dir $VAL_DIR --num_epochs $EPOCHS_2 --batch_size $BATCH_SIZE --learning_rate 0.01 --image_height $IMAGE_HEIGHT --image_width $IMAGE_WIDTH --checkpoint_dir $CKPT --checkpoint_file $CKPT_PATH --log_file $LOG --continue_model $CKPT_FILE 
