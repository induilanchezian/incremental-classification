...
def model(input_shape):
  ...
  model.add(Flatten())
  model.add(Dense(1024, kernel_initializer=Orthogonal(1.0), bias_initializer=Constant(0.1), kernel_regularizer=regularizers.l2(0.0005)))
  model.add(LeakyReLU(0.01))
  model.add(Lambda(lambda x : maxout(x, 512), lambda shp : maxout_shape(shp, 512)))
