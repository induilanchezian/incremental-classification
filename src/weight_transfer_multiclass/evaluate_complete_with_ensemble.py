...
from model_binary import model as m_bin
...
def predict_binary(test_data_dir, checkpoint, shp, batch_size):
...
def evaluation_report(test_data_dir, checkpoints_list, report_file, batch_size, input_shape, extract_features=False, feature_file=None, roc_file=None):
  ...
  y_pred = y_pred.astype(int)
  y_pred = y_pred + 1
  predictions_stage_0[predictions_stage_0 > 0] = y_pred[predictions_stage_0 > 0]
