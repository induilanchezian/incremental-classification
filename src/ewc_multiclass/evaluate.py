...
from ewc_incremental_train import ewc_loss, maxout, maxout_shape, accuracy 
...
def evaluation_report(data_dir,checkpoint, report_file, batch_size, input_shape, extract_features=False, feature_file=None, roc_file=None):
  ...
  def_model = load_model('../../models/base/modelv1-KaggleDRD-50ep-base.h5')
  outputTensor = def_model.output
  if 'ewc' in checkpoint:
    model = load_model(checkpoint, custom_objects={'<lambda>': lambda y_true, y_pred: ewc_loss(y_true, y_pred, def_model.layers, def_model.layers, outputTensor, lambda_const)})
    print('EWC')
  else:
...
if __name__ == '__main__':
  ...
  parser.add_argument('--roc_file', type=str, default=None, help='File where ROC plot is saved')
