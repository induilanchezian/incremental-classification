...
WEIGHTS_PATH=weights_multiclass_smaller_ewc_convfix
LOG=losses_1.txt

REPORT=multiclass-v1-ewc-test-report.json
ROC=multiclass-v1-ewc-test-ROC.png

TRAIN_REPORT=multiclass-v1-ewc-train-report.json
TRAIN_ROC=multiclass-v1-ewc-train-ROC.png

BASE_TRAIN_REPORT=multiclass-v1-ewc-inc-train-report.json
BASE_TRAIN_ROC=multiclass-v1-ewc-inc-train-ROC.png

LAMBDA=0.1
DEVICE=0

#train using ewc on incremental data
CUDA_VISIBLE_DEVICES=$DEVICE python ewc_convfix_incremental_train.py --train_dir $TRAIN_DIR --validation_dir $VAL_DIR --prev_model $PREV_MODEL --num_epochs $EPOCHS --batch_size $BATCH_SIZE --reg_const $LAMBDA --image_height $IMAGE_HEIGHT --image_width $IMAGE_WIDTH --checkpoint_dir $CKPT_DIR --checkpoint_file $WEIGHTS_PATH --log_file $LOG
